...
def trigger_wink(slot_id=None):
    ...
    okay = authenticator_app.yes_no_prompt("Sign?", font=vga2_bold_16x32)
    tidal.led[0] = (0, 0, 0)
    tidal.led.write()
...
def allow_interrupt_when_authenticating():
    def check_for_wink():
        ...
        if requested == False: # Slot 0 might be requested, which is falsey
            return
        elif prompting:
    ...
    scheduler.periodic(5000, check_for_wink)